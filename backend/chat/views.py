...
class UpdateChatRequestStatusAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            chat_request_one_side = ChatInterestUser.objects.get(raised_user_id=raised_user_id, opt_user=request.user.id)
            chat_request_other_side = ChatInterestUser.objects.get(raised_user_id=request.user.id, opt_user=raised_user_id)
